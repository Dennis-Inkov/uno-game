...
exports.handleRoundWin = handleRoundWin;
exports.scheduleBotTurn = scheduleBotTurn;
...
function handleRoundWin(io, room, winnerId) {
    const state = room.game;
    const winner = room.players.find(p => p.id === winnerId);
    const roundPoints = (0, GameEngine_1.calcRoundScore)(room.players);
...
function scheduleBotTurn(io, room) {
    ...
    setTimeout(() => {
        if (!room.game || room.game.winner)
            return;
        if (room.game.pendingColorChoice)
            return;
        ...
        if (room.game.lastHandBeforeWD4 !== null && room.game.drawAccumulator >= 4) {
            ...
            if (previousPlayer && Math.random() < 0.3) {
                ...
                if (couldPlay) {
                    (0, GameEngine_1.drawCards)(room.game, previousPlayer, 4);
                    ...
                    (0, GameEngine_1.drawCards)(room.game, current, 6);
                    room.game.drawAccumulator = 0;
                    io.to(room.code).emit('game:event', {
                        type: 'challenge_fail', actorName: current.name, ts: Date.now(),
                    });
                    (0, GameEngine_1.advanceTurn)(room.game, room.players);
                }
                broadcastState(io, room);
            ...
            (0, GameEngine_1.drawCards)(room.game, current, room.game.drawAccumulator);
            room.game.drawAccumulator = 0;
            room.game.lastHandBeforeWD4 = null;
            (0, GameEngine_1.advanceTurn)(room.game, room.players);
            broadcastState(io, room);
        ...
        const action = (0, BotPlayer_1.botChooseAction)(current.hand, room.game);
        if (action.type === 'draw') {
            if (room.game.drawAccumulator > 0) {
                ...
                (0, GameEngine_1.drawCards)(room.game, current, 1);
            }
            (0, GameEngine_1.advanceTurn)(room.game, room.players);
        }
        else {
            ...
            const { skipExtra } = (0, GameEngine_1.applyCardEffect)(action.card, action.chosenColor, room.game, room.players);
            if ((0, GameEngine_1.checkWin)(current)) {
                handleRoundWin(io, room, current.id);
                return;
            }
            if ((0, GameEngine_1.checkUno)(current)) {
                (0, BotPlayer_1.scheduleUnoCall)(current, () => {
            ...
            (0, GameEngine_1.advanceTurn)(room.game, room.players, skipExtra);
        }
        broadcastState(io, room);
        const nextCurrent = room.players[room.game.currentPlayerIndex];
        if (nextCurrent?.isBot)
            scheduleBotTurn(io, room);
    }, delay);
}
function registerGameHandlers(io, socket) {
    socket.on('game:play-card', ({ cardId, chosenColor }) => {
        ...
        if (!(0, GameEngine_1.isValidPlay)(card, state, currentPlayer)) {
        ...
        const { skipExtra, pendingColorChoice } = (0, GameEngine_1.applyCardEffect)(card, chosenColor, state, room.players);
        if ((0, GameEngine_1.checkWin)(currentPlayer)) {
            handleRoundWin(io, room, currentPlayer.id);
            return;
        }
        if (!pendingColorChoice) {
            (0, GameEngine_1.advanceTurn)(state, room.players, skipExtra);
        }
        broadcastState(io, room);
        if (!pendingColorChoice) {
    ...
    socket.on('game:draw-card', () => {
        ...
        (0, GameEngine_1.drawCards)(state, currentPlayer, count);
        (0, GameEngine_1.advanceTurn)(state, room.players);
        broadcastState(io, room);
        const nextCurrent = room.players[state.currentPlayerIndex];
        if (nextCurrent?.isBot)
            scheduleBotTurn(io, room);
    ...
    socket.on('game:say-uno', () => {
        ...
        if (!room || !room.game)
            return;
        const player = (0, RoomManager_1.getPlayer)(room, socket.id);
        if (!player)
            return;
    ...
    socket.on('game:catch-uno', () => {
        ...
        if (!room || !room.game || room.game.winner)
            return;
        const catcher = (0, RoomManager_1.getPlayer)(room, socket.id);
        if (!catcher)
            return;
        ...
        (0, GameEngine_1.drawCards)(room.game, victim, 2);
        io.to(room.code).emit('game:event', {
            type: 'catch_uno',
    ...
    socket.on('game:challenge-draw4', () => {
        ...
        if (!room || !room.game)
            return;
        const state = room.game;
        const challenger = (0, RoomManager_1.getPlayer)(room, socket.id);
        if (!challenger)
            return;
        if (state.currentPlayerIndex !== room.players.findIndex(p => p.id === socket.id))
            return;
        if (!state.lastHandBeforeWD4)
            return;
        ...
        state.lastHandBeforeWD4 = null;
        if (couldPlay) {
            (0, GameEngine_1.drawCards)(state, previousPlayer, 4);
            state.drawAccumulator = 0;
            io.to(room.code).emit('game:event', {
            ...
            (0, GameEngine_1.drawCards)(state, challenger, 6);
            state.drawAccumulator = 0;
            io.to(room.code).emit('game:event', {
